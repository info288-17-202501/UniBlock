    ...
    <div className="min-h-screen lg:pt-30 bg-white p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 mt-8">
          <h2 className="text-xl font-semibold mb-6 text-gray-800">Formulario de elección - PASO {currentStep}/4</h2>

          {currentStep === 1 && (
            <Step1 nextStep={nextStep} formData={formData} setFormData={setFormData} />
          )}
          {currentStep === 2 && (
            <Step2 nextStep={nextStep} prevStep={prevStep} formData={formData} setFormData={setFormData} />
          )}
          {currentStep === 3 && (
            <Step3 nextStep={nextStep} prevStep={prevStep} formData={formData} setFormData={setFormData} />
          )}
          {currentStep === 4 && (
...
function ProgressBar({ currentStep }) {
  return (
    <div className="relative">
      <div className="flex items-center justify-between">
        {[1, 2, 3, 4].map(step => (
          <div key={step} className="flex flex-col items-center z-10">
            <div className={`w-8 h-8 flex items-center justify-center rounded-full ${currentStep >= step ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'} font-medium`}>
              {step}
            </div>
          </div>
        ))}
      ...
      <div className="absolute top-4 left-0 right-0 flex px-8">
        <div className={`flex-1 h-0.5 mx-2 transition-all duration-300 ${currentStep >= 2 ? 'bg-blue-600' : 'bg-transparent'}`}></div>
        <div className={`flex-1 h-0.5 mx-2 transition-all duration-300 ${currentStep >= 3 ? 'bg-blue-600' : 'bg-transparent'}`}></div>
        <div className={`flex-1 h-0.5 mx-2 transition-all duration-300 ${currentStep >= 4 ? 'bg-blue-600' : 'bg-transparent'}`}></div>
      </div>
    </div>
  );
}
...
function Step2({ nextStep, prevStep, formData, setFormData }) {
  const addCandidate = () => {
    ...
    if (name) {
      setFormData(prev => ({ ...prev, candidates: [...prev.candidates, name] }));
    }
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex justify-between mt-6">
        <button className="btn-outline" onClick={prevStep} type="button">Volver</button>
        <button className="btn-primary" onClick={nextStep} type="button">Siguiente</button>
      </div>
    </div>
  );
}
...
function Step3({ nextStep, prevStep, formData, setFormData }) {
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h3 className="text-sm font-medium text-gray-700 mb-3">¿Quién puede votar?</h3>
        <div className="space-y-3">
          <label className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
            ...
            />
            <div>
              <span className="block text-sm font-medium">Todos los usuarios</span>
              <span className="block text-xs text-gray-500">Cualquier persona con acceso al sistema podrá votar</span>
            </div>
          </label>
          <label className="flex items-center gap-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
            <input 
              ...
              onChange={() => setFormData({ ...formData, users: 'restringido' })}
            />
            <div>
              <span className="block text-sm font-medium">Usuarios restringidos</span>
              <span className="block text-xs text-gray-500">Solo usuarios seleccionados podrán votar</span>
            </div>
          </label>
        </div>
      </div>
...
function Step4({ prevStep, formData }) {
  return (
    <div className="flex flex-col gap-4">
      <h3 className="text-lg font-medium text-gray-900 mb-2">Resumen de la elección</h3>
      ...
      <div className="flex justify-between mt-8">
        <button className="btn-outline" onClick={prevStep} type="button">Volver</button>
        <button className="btn-success" type="button">Confirmar y crear elección</button>
      </div>
    </div>
  );
}
